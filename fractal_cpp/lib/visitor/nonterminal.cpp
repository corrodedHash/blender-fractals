...
namespace frac {
NonTerminalTree
NonTerminal::get_tree(unsigned int depth) {
  return NonTerminalTree(*this, depth);
}

void
NonTerminalTree::iterator::findTerminal() {
  while (not iterator_stack.empty() and
         (iterator_stack.top().finished() or
          not iterator_stack.top().cur_is_term())) {
...
NonTerminalTree::iterator&
NonTerminalTree::iterator::operator++() {
  if (iterator_stack.empty()) {
    return *this;
  }
...
void
NonTerminalTree::iterator::decreaseCurrentLevel() {
  iterator_stack.pop();
}

NonTerminalTree::iterator
NonTerminalTree::begin() {
  if (depth == 0) {
