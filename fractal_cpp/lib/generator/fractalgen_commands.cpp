...
namespace frac {
void
FractalGen::move(FType distance) {
  endface();
  moved = true;
...
void
FractalGen::draw(FType distance) {
  endface();
  if (moved) {
    moved = false;
...
void
FractalGen::face(FType distance) {
  if (moved) {
    moved = false;
    fractal.add_vert(position_stack.top());
    verts_stack.top() = fractal.vert_count() - 1;
...
void
FractalGen::rotate(const std::array<FType, 3>& rotation) {
  // Rotation around local y axis
  if (rotation[0] != 0) {
    frac::axis_rotate(rotation_stack.top(), look_at_stack.top(), rotation[0],
                      rotation_stack.top());
  }
  // Rotation around local z axis
  if (rotation[1] != 0) {
    std::valarray<FType> rot_axis =
        frac::cross(rotation_stack.top(), look_at_stack.top());
    frac::axis_rotate(rotation_stack.top(), rot_axis, rotation[1],
                      rotation_stack.top());
    frac::axis_rotate(look_at_stack.top(), rot_axis, rotation[1],
                      look_at_stack.top());
  }
  if (rotation[2] != 0) {
    frac::axis_rotate(look_at_stack.top(), rotation_stack.top(), rotation[0],
                      look_at_stack.top());
  }
}
