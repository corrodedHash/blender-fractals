...
static frac::NonTerminalManager
parseGrammar(antlr4::ANTLRInputStream istream) {
...
static frac::NonTerminalManager
parseGrammar(const std::string& filename) {
...
static frac::NonTerminalManager
parseStringGrammar(const char data[], size_t size) {
  return parseGrammar(antlr4::ANTLRInputStream(data, size));
}

static frac::FractalGen
generateFractal(::frac::NonTerminalManager ntm, unsigned int level) {
...
::frac::FractalGen
frac::generateFractal(const std::string& filename, unsigned int level) {
  return ::generateFractal(parseGrammar(filename), level);
}

::frac::FractalGen
frac::generateFractal(const char data[], size_t size, unsigned int level) {
  return ::generateFractal(parseStringGrammar(data, size), level);
}

::frac::MeshInfo
frac::generateMesh(const std::string& filename, unsigned int level) {
