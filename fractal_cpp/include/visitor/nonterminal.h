...
public:
  ...
  NonTerminal(std::string _name) : name(std::move(_name)){};

  // TODO: Should make a map to allow multiple transitions
  void addTrans(NTHolder _trans) { trans = std::move(_trans); }
  void addFinalTrans(NTHolder _final_trans) {
    final_trans = std::move(_final_trans);
  }
...
struct FlatNonterminal {
  const NTHolder& nt;
  unsigned int index;
  FlatNonterminal(const NTHolder& nt_) : nt(nt_), index(0) {}
  bool finished() const { return index >= nt.size(); }
  bool cur_is_term() const { return nt.isTerminal(index); }
...
class NonTerminalTree::iterator :
    public std::iterator<std::output_iterator_tag, Terminal> {
private:
...
public:
  ...
  explicit iterator(unsigned int depth) { wanted_level = depth; };
  const Terminal& operator*() const;
  iterator& operator++();
  bool operator==(const iterator& other) const {
    return wanted_level == other.wanted_level and
           iterator_stack == other.iterator_stack;
  }
