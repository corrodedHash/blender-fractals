...
cdef extern from "generator/fractalgen.h" namespace "frac":
    cdef cppclass mesh_info:
        ...
        uint64_t * face_starts
        uint64_t * face_totals
...
def generate_fractal(filename, level):
    ...
    if (bla.face_vert_size > 0):
      ...
      face_starts_array = <uint64_t[:bla.face_bound_size]> bla.face_starts
      face_totals_array = <uint64_t[:bla.face_bound_size]> bla.face_totals
